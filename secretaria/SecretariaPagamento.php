<?php
    ...
    $valorCompraTotal = $dadosPagamento['valor_total']; // Valor total da compra
    ...
    if (!$stmtSecretaria) {
        die("Erro na preparação da consulta da secretaria: " . $conn->error);
    }
    ...
    $clienteNome = 'Cliente não informado'; // Valor padrão
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($formaPagamento)) {
            ...
            foreach ($carrinho as $item) {
                ...
                $stmtInsert->bind_param("isdsii", $secretariaId, $cpfParaInsert, $valorCompraItem, $formaPagamento, $item['id_produto'], $item['quantidade']);
        ...
        <div class="pagamento">
            <div class="pag">
                <div class="info-resumo">
                    ...
                    <p><strong>Valor Total:</strong> R$ <?php echo number_format($valorCompraTotal, 2, ',', '.'); ?></p>
                </div>
                <div class="CardDin">
                    <nav>
                        <a href="#" id="selec" class="active-link">Cartão</a>
                        <a href="SecretariaPagamentoDinheiro.php" class="hover-underline">Dinheiro</a>
                    </nav>
                </div>
                <?php if (!empty($erro)): ?>
                ...
                <form method="POST" action="">
                    <input 
                        type="hidden" 
                        name="valor" 
                        value="<?php echo htmlspecialchars($valorCompraTotal); ?>" />
                        ...
                        <div class="PrimLin">
                            <div class="valor">
                                <input 
                                    ...
                                    name="valor_display" 
                                    placeholder="Valor:" 
                                    value="R$ <?php echo number_format($valorCompraTotal, 2, ',', '.'); ?>" 
                                    readonly>
                            </div>
